...
class MainWindow(gtk.Window):
    ...
    def runCapture(self):
        ...
        writer1 = cv.CreateVideoWriter("video-out.avi",cv.CV_FOURCC("F","L","V","1"),15,(width,height),1)
        writer2 = cv.CreateVideoWriter("kinect-out.avi",cv.CV_FOURCC("F","L","V","1"),15,(width,height),1)
        p.setData(1)
        ...
        while (loop):
            ...
            cv.WriteFrame(writer, cv.fromarray(rgb));
            cv.WriteFrame(writer2, np.dstack((depth,depth,depth)).astype(np.uint8));
            if StopEvent.is_set():
                loop = False
            mov = mov+1
